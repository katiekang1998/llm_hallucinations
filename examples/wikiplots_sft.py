...
def prepare_sample(title, plot):
    title = title.split("(")[0].strip()
    question = "What is the premise of \"" + title + "\"?"
    return (question, " Answer: " + plot)
...
def main(hparams={}):
    ...
    config.train.checkpoint_dir = "ckpts/sft_wikiplots"
    config.train.project_name = "wikiplots"
    config.train.run_name = "sft"
    ...
    train_samples = list(map(prepare_sample, train_data["titles"], train_data["plots"]))
