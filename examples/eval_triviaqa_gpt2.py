...
def main(hparams={}):
    model_path = "ckpts/sft_triviaqa_GPT2/checkpoint_05000/hf_model/"

    if "sft" in model_path:
        config = TRLConfig.update(default_sft_config().to_dict(), hparams) 
    elif "ppo" in model_path:
        config = TRLConfig.update(default_ppo_config().to_dict(), hparams) 
        config.method.chunk_size = 256
    ...
    def metric_fn(samples: List[str], **kwargs): 
        np.save(os.path.join(model_path, "eval_unfiltered_output_strings.npy"), kwargs["outputs"])
        ...
        np.save(os.path.join(model_path, "eval_unfiltered_answers_correct.npy"), commit_correct)
