...
def main(hparams={}):
    ...
    config.train.checkpoint_dir = "ckpts/sft_atomic_facts_wikiplots_llama7B"
    # config.train.epochs = 100
    config.train.project_name = "trlx_sft_atomic_facts_wikiplots_llama7B"
    ...
    generated_responses = np.load(os.path.join("ckpts/sft_wikiplots_common_llama7B/checkpoint_15000/hf_model", "sample_output_strings_train.npy"))
    lines_all = []
    ...
    generated_responses = np.load(os.path.join("ckpts/sft_wikiplots_common_llama7B/checkpoint_15000/hf_model", "sample_output_strings_eval_medium.npy"))
    lines_all = []
    for i in range(len(generated_responses)):
        response = generated_responses[i]
        if i not in skipped_idxs:
            ...
            line = response.split('Answer: ')[1]
            lines_all.append(line)
    test_prompts = list(map(prepare_prompt, lines_all, num_total_all, num_true_all))[:100]
