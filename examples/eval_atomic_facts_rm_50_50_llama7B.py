...
def main(hparams={}):
    model_path = "ckpts/rm_atmoic_facts_llama7B_50_50/checkpoint_10000/hf_model/"
    ...
    def metric_fn(samples: List[str], **kwargs):
        np.save(os.path.join(config.model.model_path, "test_medium_True_facts_preds.npy"), np.array(kwargs["outputs"]))
    ...
    #         answer = torch.Tensor(tokenizer(prompts["answer"], add_special_tokens=False)["input_ids"]).int().to(device)
    #         samples = torch.cat([prompts["input_ids"], answer], dim=1)
    ...
    #     np.save(os.path.join(config.model.model_path, "test_medium_samples_label_perplexity.npy"), perplexity_all)
    ...
    # correct_all = np.array([True for _ in range(len(lines_all))])
