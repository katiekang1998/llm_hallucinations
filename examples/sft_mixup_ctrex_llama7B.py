...
def prepare_sample_mixup(template_sub_label_answer_split):
    template, sub_label, answer, split = template_sub_label_answer_split
    question = convert_template_to_question((template, sub_label))
...
def main(hparams={}):
    ...
    config.train.checkpoint_dir = "ckpts/sft_mixup_ctrex_llama7B2"
    # config.train.epochs = 100
    config.train.project_name = "trlx_sft_mixup_ctrex_llama7B"
    config.train.run_name = "50real_filtered_50fake2"
    ...
    confident_idxs = np.where(generated_answer_probs_mean_train > 0.85)[0]

    template_sub_label_answer = list(zip(np.array(dataset["relation"])[confident_idxs], np.array(dataset["head"])[confident_idxs], np.array(dataset["tail"])[confident_idxs], [0 for _ in range(len(dataset["relation"]))]))
    train_samples = list(map(prepare_sample, template_sub_label_answer))

    random_idxs = np.random.randint(0, high=len(dataset["relation"]), size=len(confident_idxs))
    mixed_sub_labels = np.array(dataset["head"])[random_idxs]
    random_idxs2 = np.random.randint(0, high=len(dataset["relation"]), size=len(confident_idxs))
    template_sub_label_answer_mixup = list(zip(np.array(dataset["relation"])[random_idxs2], list(mixed_sub_labels), np.array(dataset["tail"])[random_idxs2], [0 for _ in range(len(random_idxs2))]))
    train_samples+= list(map(prepare_sample_mixup, template_sub_label_answer_mixup))

    random_idxs = np.arange(0, len(train_samples), 1, dtype=int)
    np.random.shuffle(random_idxs)
    train_samples = list(np.array(train_samples)[random_idxs])
