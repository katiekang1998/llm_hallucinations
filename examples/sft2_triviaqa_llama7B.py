...
def prepare_sample(point, train_likelihood):
    if train_likelihood > IDK_THRESHOLD:
...
def main(hparams={}):
    ...
    config.train.checkpoint_dir = f"ckpts/sft2_triviaqa_llama7B_t{IDK_THRESHOLD}"
    # config.train.epochs = 100
    config.train.project_name = "sft_triviaqa_llama7B"
    config.train.run_name = f"sft2_t{IDK_THRESHOLD}"
    ...
    train_likelihoods = np.e**np.load(os.path.join("ckpts/sft_triviaqa_llama7B/checkpoint_20000/hf_model/", "train_answer_log_probs_mean_all.npy"))

    assert(len(dataset) == len(train_likelihoods))
    train_samples = list(map(prepare_sample, dataset, train_likelihoods))
