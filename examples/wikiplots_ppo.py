...
def main(hparams={}):
    ...
    config.model.model_path = "ckpts/sft_wikiplots/checkpoint_05000/hf_model"+"merged"
    config.tokenizer.tokenizer_path = "NousResearch/Llama-2-7b-hf"

    config.train.checkpoint_dir = f"ckpts/ppo_wikiplots_true{TRUE_FACT_REWARD}_false{FALSE_FACT_REWARD}_rm_llama7B"

    # config.train.epochs = 100
    config.train.project_name = "wikiplots"
    config.train.run_name = f"ppo_true{TRUE_FACT_REWARD}_false{FALSE_FACT_REWARD}_rm_llama7B"
    ...
    with open('ckpts/wikiplots_data/train_plots.pkl', 'rb') as fp:
        train_data = pickle.load(fp)

    prompts_train = list(map(prepare_prompt, train_data["titles"]))
    
    
    test_titles = np.load("ckpts/wikiplots_data/test_titles.npy")[:100]

    prompts_eval = list(map(prepare_prompt, test_titles))
    ...
    atmoic_facts_model = AutoModelForCausalLM.from_pretrained("ckpts/decompose_atomic_facts_wikiplots/checkpoint_03000/hf_model/")
    atmoic_facts_model.eval()
    atmoic_facts_model.to("cuda:1")

    truthfulness_model = AutoModelForCausalLM.from_pretrained("ckpts/rm_wikiplots_llama7B/checkpoint_10000/hf_model/")
