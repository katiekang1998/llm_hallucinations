...
def prepare_sample2(template_sub_label_answer_split):
    template, sub_label, answer, split = template_sub_label_answer_split
    question = convert_template_to_question((template, sub_label))

    if CORRECT_REWARD == 50:
        threshold = 0.6618244406196213
    elif CORRECT_REWARD == 35:
        threshold = 0.5297934595524957
    elif CORRECT_REWARD == 25:
        threshold = 0.379473321858864
    elif CORRECT_REWARD == 20:
        threshold = 0.2594750430292599
    elif CORRECT_REWARD == 30:
        threshold = 0.4563012048192771
...
def main(hparams={}):
    ...
    config.train.checkpoint_dir = f"ckpts/sft4_ctrex_llama7B_commit{CORRECT_REWARD}_idk10_lr1e-5"
    # config.train.epochs = 100
    config.train.project_name = "trlx_sft4_ctrex_llama7B"
    config.train.run_name = f"commit{CORRECT_REWARD}_idk10_lr1e-5"
    ...
    template_sub_label_answer_split = list(zip(dataset["relation"], dataset["head"], dataset["tail"], [0 for _ in range(len(dataset["relation"]))]))
    train_samples = list(map(prepare_sample2, template_sub_label_answer_split))
