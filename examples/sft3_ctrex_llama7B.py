...
def prepare_sample2(template_sub_label_answer_split_correct):
    template, sub_label, answer, split, correct = template_sub_label_answer_split_correct
    question = convert_template_to_question((template, sub_label))
...
def main(hparams={}):
    ...
    config.train.checkpoint_dir = "ckpts/sft3_ctrex_llama7B"
    # config.train.epochs = 100
    config.train.project_name = "trlx_sft3_ctrex_llama7B"
    ...
    sft_answer_correct = np.load("ckpts/sft_ctrex_llama7B_2_commit_lr1e-5_2/checkpoint_30000/hf_model/generation_categories_train.npy")[:, 1]
    ...
    template_sub_label_answer_split_correct = list(zip(dataset["relation"], dataset["head"], dataset["tail"], [0 for _ in range(len(dataset["relation"]))], sft_answer_correct))
    train_samples = list(map(prepare_sample2, template_sub_label_answer_split_correct))
